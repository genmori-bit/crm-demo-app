...
const RATING_MAP: Record<string, { label: string; variant: "danger" | "warning" | "info" | "muted" }> = {
...
const LIST_VIEWS = [
  ...
  { id: "working",      label: "対応中",  status: "WORKING" },
  { id: "nurturing",    label: "育成中",  status: "NURTURING" },
  { id: "converted",    label: "変換済",  status: "CONVERTED" },
];
...
function LeadsInner() {
  ...
  const status = LIST_VIEWS.find((v) => v.id === activeView)?.status ?? "";
        ...
        <table className="w-full text-sm" role="grid">
          <thead>
            <tr className="bg-sf-bg border-b border-sf-border sticky top-0 z-10">
              ...
              <th className="text-left px-4 py-2.5 text-xs font-semibold text-sf-weak uppercase tracking-wider whitespace-nowrap">会社・役職</th>
              ...
              <th className="text-left px-4 py-2.5 text-xs font-semibold text-sf-weak uppercase tracking-wider whitespace-nowrap">評価</th>
              <th className="text-left px-4 py-2.5 text-xs font-semibold text-sf-weak uppercase tracking-wider whitespace-nowrap">スコア</th>
              <th className="text-left px-4 py-2.5 text-xs font-semibold text-sf-weak uppercase tracking-wider whitespace-nowrap">ソース</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={6}>
                  <EmptyState
                    ...
                    action={<Button onClick={() => router.push("/leads/new")}>新規リード作成</Button>}
                    icon={
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    {ratingInfo ? (
                      <Badge variant={ratingInfo.variant}>{ratingInfo.label}</Badge>
                    ) : <span className="text-sf-weak text-xs">—</span>}
                  </td>
                  <td className="px-4 py-3">
                    {lead.score != null ? (
                      <span className="text-sm font-semibold tabular-nums text-sf-text">{lead.score}</span>
                    ) : <span className="text-sf-weak text-xs">—</span>}
                  </td>
