...
export function Sidebar() {
  ...
  useEffect(() => {
    api.get<Dashboard[]>("/api/dashboards?filter=all")
      .then(setDashboards)
      .catch(() => {});
  }, []);

  const isDashboardActive = pathname.startsWith("/dashboards");
        ...
        <div className="pt-1">
          <div className="flex items-center justify-between px-3 py-2 rounded-lg group">
            <button
              ...
              className={cn(
                "flex items-center gap-3 text-sm font-medium flex-1 min-w-0 transition-colors",
                isDashboardActive ? "text-white" : "text-gray-400 hover:text-white"
              )}
            >
              ...
              <svg
                className={cn("h-3.5 w-3.5 shrink-0 transition-transform ml-auto", dashboardOpen ? "rotate-180" : "")}
            ...
            <Link
              href="/dashboards/new"
              className="ml-1 p-0.5 text-gray-500 hover:text-white transition-colors shrink-0"
              title="新規ダッシュボード"
            >
            ...
            <div className="ml-4 mt-0.5 border-l border-gray-700 pl-3 space-y-0.5">
              ...
              {dashboards.slice(0, 8).map((d) => (
                <Link
                  ...
                  className={cn(
                    "flex items-center px-2 py-1.5 rounded text-xs transition-colors truncate",
                    pathname === `/dashboards/${d.id}`
                      ? "text-white bg-gray-800"
                      : "text-gray-500 hover:text-gray-200 hover:bg-gray-800"
                  )}
                  title={d.name}
              ...
              {dashboards.length === 0 && (
                <p className="px-2 py-1.5 text-xs text-gray-600">ダッシュボードなし</p>
              )}
            </div>
          )}
        </div>
